...
export const StructuredDialog = ({
  ...
  return (
    <Dialog className={buildClassName(className, 'jtjs-structured-dialog')} {...otherProps} ref={internalRef}>
      <Flexbox direction="column">
        ...
        {buttons.length > 0 && (
          <Flexbox
            ...
            spacing="3rem"
          >
            {buttons.map(({ text, buttonProps = {}, closeDialogOnClick = false, beforeCloseOnClick }) => {
                ...
                <AsyncButton
                  ...
                  onClick={async (...args) => {
                    ...
                    if (closeDialogOnClick) {
                      if (!beforeCloseOnClick || (await beforeCloseOnClick())) {
