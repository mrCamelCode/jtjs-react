...
export const Flexbox = ({
  ...
  const flexWrap = `${wrap ? 'wrap' : 'nowrap'}${reverseWrap && wrap ? '-reverse' : ''}`;
  const flexFlow = `${direction}${reverseDirection ? '-reverse' : ''} ${flexWrap}`;
  ...
  switch (horizontalAlignment) {
    case 'left':
      horizontalAligmentToFlexTerms = mainAxis === 'x' && reverseDirection ? 'flex-end' : 'flex-start';
      break;
    case 'center':
      horizontalAligmentToFlexTerms = 'center';
      break;
    case 'right':
      horizontalAligmentToFlexTerms = mainAxis === 'x' && reverseDirection ? 'flex-start' : 'flex-end';
      break;
  }
  ...
  switch (verticalAlignment) {
    case 'top':
      verticalAlignmentToFlexTerms = mainAxis === 'y' && reverseDirection ? 'flex-end' : 'flex-start';
      break;
    case 'center':
      verticalAlignmentToFlexTerms = 'center';
      break;
    case 'bottom':
      verticalAlignmentToFlexTerms = mainAxis === 'y' && reverseDirection ? 'flex-start' : 'flex-end';
      break;
  }

  // justify-content drives the alignment and spacing along the main axis.
  const justifyContent = mainAxis === 'x' ? horizontalAligmentToFlexTerms : verticalAlignmentToFlexTerms;

  // align-items drives the alignment along the cross axis
  const alignItems = crossAxis === 'x' ? horizontalAligmentToFlexTerms : verticalAlignmentToFlexTerms;

  // align-content drives the flex container's lines when multiline, and drives spacing and alignment along the
  // cross axis.
  const alignContent = crossAxis === 'x' ? horizontalAligmentToFlexTerms : verticalAlignmentToFlexTerms;
